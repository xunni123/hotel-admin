...
export function useTable(api: TableOptions, options: UseTableOptions = {}) {
 ...
 const updateItem = async (rowIdx: number, newRow: Record<string, any>, oldRow: Record<string, any>, getId: (row: Record<string, any>) => number) => {
 if (!api.update)
 return;
 loading.value = true;
 try {
 ...
 const deleteItem = async (row: Record<string, any>, getId: (row: Record<string, any>) => number) => {
 if (!api.delete)
 return;
 loading.value = true;
 try {
