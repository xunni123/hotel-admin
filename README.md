...
+-- README.md                               本文档
+-- config/
|   +-- application-prod.yml                生产环境配置样例
+-- database/                               建库建表与初始化数据 SQL
|   +-- hotel_management.sql                核心业务表结构
|   +-- init_additional_tables.sql          补充表
|   +-- orders.sql member_consume.sql       订单与会员消费表
|   +-- init_goods.sql init_announcements.sql init_new_menus.sql
|   +-- cleaning_tables.sql financial_record.sql
+-- nginx-config/                           Nginx 站点配置样例
|   +-- 39.108.212.17.conf
|   +-- proxy-api.conf
+-- test/                                   测试环境配置样例
|   +-- nginx/                              Nginx 测试配置
...
|   +-- pom.xml                             父 POM，版本锁定与模块聚合
|   +-- deploy.sh                           部署脚本
|   +-- DEPLOY.md                           部署文档
|   +-- mvnw mvnw.cmd .mvn/wrapper          Maven Wrapper
|   +-- sql/
|   |   +-- check_and_query.sql             字典表查询与建库后数据校验
|   |   +-- simple_queries.sql              简化版查询 SQL
|   +-- hotel-common/                       ==== 公共模块 ====
|   |   +-- pom.xml
|   |   +-- src/main/java/com/xunni/hotel/
|   |       +-- common/
|   |       |   +-- result/Result.java      统一响应 Result 泛型
|   |       |   +-- utils/JwtUtil.java      JWT 工具类
|   |       +-- entity/
|   |           +-- HomeCard/               仪表盘复合 DTO，共 7 个
|   |           +-- 17 个核心业务实体        User Room Order Member Booking 等
|   +-- hotel-web/                          ==== Web 模块 ====
|       +-- pom.xml
|       +-- src/main/
|           +-- java/com/xunni/hotel/web/
|           |   +-- HotelApplication.java   Spring Boot 启动类
|           |   +-- config/                 SecurityConfig CorsConfig
|           |   |                           RedisConfig PasswordEncoderConfig
|           |   |                           GlobalExceptionHandler
|           |   +-- controller/             14 个业务 Controller
|           |   +-- controller/role/        5 个角色权限相关 Controller
|           |   +-- dto/                    登录请求响应等 DTO
|           |   +-- mapper/                 14 个业务 Mapper 接口
|           |   +-- mapper/role/            AddRoleMapper RoleMenuMapper
|           |   |                           SelectRoleMapper
|           |   +-- service/                Service 接口
|           |   +-- service/iml/            Service 实现，iml 目录
|           |   +-- service/impl/           GoodsService 实现，impl 目录
|           |   +-- service/role/           RoleService addRoleService
|           +-- resources/
|               +-- application.yml         主配置，JWT 与 Jackson
|               +-- application-active.yml  本地开发环境
|               +-- application-test.yml    测试环境
|               +-- application-prod.yml    生产环境
|               +-- application.properties  spring.application.name
|               +-- mapper/RoomMapper.xml   自定义 SQL 示例
|               +-- logback-spring.xml      日志配置
|
+-- hotel-web/                              ====== 前端：Vue 3 + Vite + TypeScript ======
    +-- .env.development  .env.production   前端环境变量
    +-- vite.config.ts                      代理 自动引入 gzip 手动分包
    +-- tsconfig.json tsconfig.app.json tsconfig.node.json
    +-- auto-imports.d.ts components.d.ts   unplugin 自动生成
    +-- index.html
    +-- package.json
    +-- public/favicon.ico config.json
    +-- src/
        +-- main.ts App.vue style.css
        +-- api/                            12 个业务模块 API 封装
        |   +-- announcement cleaning financialRecord goods
        |   +-- member memberConsume operationLog order
        |   +-- role room user booking.ts
        |   +-- dashboard.ts login.ts menus.ts
        +-- layout/                         后台整体布局
        |   +-- index.vue
        |   +-- header/ Header.vue PasswordDrawer.vue Breadcrumb Tabs
        |   +-- menu/ MenuBar MenuHeader MenuItem
        +-- router/                         index.ts 静态路由
        |   +-- auth.ts                     登录鉴权守卫
        +-- store/                          Pinia 仓库
        |   +-- login tabs roleTree roomstatus collapse checkin
        +-- views/
        |   +-- login/ 404/
        |   +-- dashboard/ 仪表盘含 HomeCard LineEcharts 等组件
        |   +-- roomstatus/ 房态面板
        |   +-- booking/list/ 预订列表
        |   +-- List/ 入住列表
        |   +-- order/ 订单管理
        |   +-- member/ 会员管理
        |   +-- memberConsume/consumeRecord/ 会员消费记录
        |   +-- room/cleaning/ 清洁管理
        |   +-- goodsInventory/goodsManage/ 商品管理
        |   +-- finance/financialReport historyOrder 财务模块
        |   +-- notice/create list 公告管理
        |   +-- base/ channel floor roomType 字典基础数据
        |   +-- system/ user role menu operationLog 系统管理模块
        +-- components/                     通用组件
        |   +-- MyTable MyDialog MyDrawer
        |   +-- Pagination SearchForm Echarts
        |   +-- MySkeleton SkeletonTable VirtualList
        |   +-- Card Loading
        +-- composables/                    组合式函数
        |   +-- useTable useLoading
        |   +-- role/useDialog useRole
        |   +-- echarts/useBarAndLineGroup useLineChart usePieChart
        |   +-- booking/useRoomOptions
        +-- types/                          TypeScript 类型定义
        +-- utils/                          cache confirm export message validation
        +-- constants/                      表单初始值 天气 首页卡片
        +-- plugins/echart.ts               ECharts 全局注册
        +-- config/index.ts                 前端全局配置
        +-- assets/                         图片 公共 scss
        +-- services/                       远程与配置服务，预留
        +-- global/                         主键策略 全局标题
```
